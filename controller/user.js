...
module.exports = {
  ...
  getUserUnregisteredUser: async (req, res, next) => {
    try {
      let user = await req.userUC.getUserUnregisteredUser();
      res.status(user.status).json(resData.success(user.data));
    } catch (e) {
      next(e);
    }
  ...
  approvedRegister: async (req, res, next) => {
    let { id } = req.params;
    let userData = {
    ...
    try {
      let resUpdate = await req.userUC.approvedRegister(userData, id);
