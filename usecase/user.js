class User {
  constructor(
    ...
    this.HomeRespository = HomeRespository;
    this.bcrypt = bcrypt;
  }
  ...
  async getUserUnregisteredUser() {
    ...
    const user = await this.UserRepository.getUserUnregisteredUser();
  ...
  async approvedRegister(userData, id) {
    let result = {
      isSuccess: false,
      reason: null,
      status: 400,
    ...
    if (user.isRegistered === true) {
      result.reason = "failed, user already active";
      return result;
    }
    let users = await this.UserRepository.getUserByHomeId(userData.homeId)
    if(users.length !== 0){
     userData.headOfFamily = false
    }
    await this.UserRepository.updateUser(userData, id);
    await this.EmailRespository.sendNotificationApprovedRegister(
      user.email,
      user
    );
